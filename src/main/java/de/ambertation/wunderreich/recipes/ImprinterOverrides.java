...
public class ImprinterOverrides {
    ...
    private static volatile Map<ResourceLocation, JsonElement> RAW = Map.of();
    ...
    public static synchronized void load(ResourceManager resourceManager) {
        final Map<ResourceLocation, JsonElement> map = new HashMap<>();
        for (Map.Entry<ResourceLocation, Resource> entry : CONVERTER.listMatchingResources(resourceManager).entrySet()) {
            final ResourceLocation enchantmentId = CONVERTER.fileToId(entry.getKey());
     ...
     */
    public static boolean isDisabled(ResourceLocation enchantmentId) {
    ...
    public static ImprinterOverride get(Holder<Enchantment> enchantment) {
        ...
        final ResourceLocation enchantmentId = enchantment.unwrapKey().get().location();
        final JsonElement element = RAW.get(enchantmentId);
